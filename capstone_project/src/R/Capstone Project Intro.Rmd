...
```{r}
pdf("host_listing_counts_reviews.pdf", width=8,height=8)
df.listing<- df.listing %>% filter((host_listings_count<10)&(host_listings_count>0)&(availability_30>0)&(availability_30<27))
ggplot(df.listing ,aes(x=as.factor(host_listings_count),y=reviews_per_month,group=host_listings_count))+ geom_boxplot(fill=airBColor)+theme(text = element_text(size=12, family="CM Roman"),legend.title=element_blank())+labs(x='listings per host',y='number of reviews per month')

dev.off()

# Embed the fonts
embed_fonts("host_listing_counts_reviews.pdf", outfile="host_listing_counts_reviews_embed.pdf")
im.convert("host_listing_counts_reviews_embed.pdf", output = "../../report/img/host_listing_counts_reviews_embed.png", extra.opts="-density 600")
```

```{r}
pdf("listing_counts.pdf", width=8,height=8)
...
g+ geom_bar(fill=airBColor)+theme(text = element_text(size=12, family="CM Roman"),legend.title=element_blank())+labs(x='listings own by host',y='total listings')


dev.off()

# Embed the fonts
embed_fonts("listing_counts.pdf", outfile="listing_counts_embed.pdf")
im.convert("listing_counts_embed.pdf", output = "../../report/img/listing_counts_embed.png", extra.opts="-density 600")
```