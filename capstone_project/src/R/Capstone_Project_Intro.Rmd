...
pdf(path$pdf, width=8,height=8)
ggplot(df.profi.appart,aes(x=price,y=city))+ geom_joy(fill = airBColor2)+theme(text = element_text(size=14, family="CM Roman"),axis.title.y=element_blank())+labs(x='Price per night($)')
...
listing.raw.summary<- df.listing.raw %>% group_by(room_type) %>% summarise(total_listing = n(), '% reviewed' =round( mean(!is.na(last_review))*100,2),'% active'=round(mean(((date_scraped-last_review)<60)&(availability_30>0),na.rm=TRUE)*100,2))

print(xtable(listing.raw.summary), include.rownames=FALSE, type = "latex")
